...
async def create_estimate(request):
    ...
    try:
        ...
        # Find customer
        customer = next(
            (c for c in customers_data if c["id"] == data.get("customerId")), None
        )
        if not customer:
        ...
        labor_rate = next(
            (
                ...
                if lr["jobType"] == data.get("jobType")
                and lr["level"] == data.get("complexity")
            ),
            None,
        )
        if not labor_rate:
        ...
        equipment_cost = 0
        selected_equipment = []
        for eq_id in data.get("equipmentIds", []):
